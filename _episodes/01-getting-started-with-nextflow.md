...
Nextflow scripts are written using a scripting language that simplifies the of writing workflows. Languages that are written for a specific fields are called Domain Specific Languages (DSL), e.g SQL, is used to work with databases, and AWK is designed for text processing.
