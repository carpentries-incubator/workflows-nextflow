...
Pipeline parameters are declared by prepending to a variable name the prefix `params`, separated by dot character e.g. `params.samples`. Their value can be specified on the command line by prefixing the parameter name with a **double dash** character, i.e. `--paramName` e.g. `--samples`
